...
1. 在聊天记录里点击某条角色回复旁边的“选中用于人设修改”。
2. 可以选择一条或多条回复，选中后消息会高亮，并出现在工作台的“当前选中的对话列表”里。
3. 在评价输入框里写评价，例如“这句太像 AI 了，不像三鹰朝，应该更别扭一点”。
4. 点击预设标签只会把标签文字追加到评价输入框，不会提交、不保存、不修改文件。
5. 点击“保存这条反馈”会把当前评价保存到 `persona_turn_feedback`。
6. 点击“发送给人设编辑 AI”会调用编辑对话接口，和一个独立的人设编辑 AI 多轮讨论怎么改。
7. 不满意可以继续补充要求，再发给人设编辑 AI。
8. 点击“生成最终修改方案”后，后端只生成最终方案和 `preview_character_json`。
9. 预览确认无误后，点击“确认应用修改”，才会写入 `character.json`。
10. 如果不满意，可以点击“回滚上一版人设”恢复固定备份。
